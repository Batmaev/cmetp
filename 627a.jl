...
md"""
Уравнение для второго подшага (3.2) решим через оператор эволюции:
```math
...
begin
	...
	Nx = 512 		# Число точек по пространству
	h  = 2L / (Nx-1)
...
# ╔═╡ 61098345-36ba-4846-a88c-db4c021d186e
md"""
Наблюдается линейный отрицательный рост фазы со временем:
"""
...
# ╔═╡ 81105f4b-e1aa-4b65-bd9f-5d24dd6a852a
md"""
Положение максимума меняется по синусоиде -- как и у классического осциллятора.

Амплитуда движения равна ``|\alpha| \sqrt2 = ``$(round(abs(α)*√2, digits = 2))
"""

# ╔═╡ 12390050-eabc-4b1c-b0f4-08df067814d0
plot(0:τ:T,
	-L - h .+ h * argmax.(eachcol(abs2.(ψ₂))),
	xlabel = "t",
	title = "Положение максимума   (при α = $α)",
	legend = nothing,
)
